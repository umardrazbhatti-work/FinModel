...
CFG_15M = ROOT / "configs" / "eurusd_rv_ma_15m.yaml"
...
def test_horizon_wall_clock_15m():
    wall = horizon_wall_clock("15m", [4, 12])
    assert wall[0]["hours"] == 1.0
    assert wall[1]["hours"] == 3.0
...
def test_15m_config_is_single_tf_rv():
    cfg = load_config(CFG_15M)
    assert cfg["data"]["pair"] == "EURUSD"
    assert cfg["data"]["tfs"] == ["15m"]
    assert cfg["data"]["primary_tf"] == "15m"
    assert cfg["data"]["tradable_tfs"] == ["15m"]
    assert cfg["data"]["target_type"] == "realized_vol"
    assert cfg["data"]["horizons"]["15m"] == [4, 12]
    assert cfg["data"]["lookback"]["15m"] == 64
    assert cfg["evaluation"]["har_windows"] == [16, 48, 96, 480]
    assert cfg["evaluation"]["require_har"] is True
    assert cfg["walk_forward"]["max_folds"] == 6
...
def test_15m_dataset_rv_forward():
    if not DATA_DIR.exists():
        pytest.skip("data missing")
    cfg = load_config(CFG_15M)
    ds = MultiTFDataset(
    ...
    cfg["data"]["context_cols"] = list(ds.context_cols)
    cfg["data"]["feature_cols"] = list(ds.feature_cols)
    cfg["data"]["tradable_tfs"] = ["15m"]
    cfg["data"]["horizons"] = {"15m": list(ds.horizons["15m"])}

    loader = DataLoader(ds, batch_size=4, collate_fn=multi_tf_collate, shuffle=False)
    batch = next(iter(loader))
    assert set(batch["inputs"].keys()) == {"15m"}
    assert set(batch["targets"].keys()) == {"15m"}
    assert batch["inputs"]["15m"].shape[1] == 64
    assert np.isfinite(batch["targets"]["15m"].numpy()).all()
    t0 = ds.debug_prediction_time(0)
    hist = ds.debug_history_timestamps(0, "15m")
    assert hist.max() <= t0

    model = SingleTFPatchModel(cfg, primary_tf="15m")
    model.eval()
    with torch.no_grad():
        out = model(batch)
    assert set(out["predictions"].keys()) == {"15m"}
    assert out["predictions"]["15m"].shape == (4, 2, 3)


def test_15m_walk_forward_six_folds():
    if not DATA_DIR.exists():
        pytest.skip("data missing")
    cfg = load_config(CFG_15M)
    ds = MultiTFDataset(
